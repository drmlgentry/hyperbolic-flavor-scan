...
fitness $0.005$ matching the theoretical minimum for this construction.
...
All computations use the unique minimal triangulation of each manifold.
For $M_\mathrm{CKM} = m006(-5,2)$: 3~tetrahedra, isometry signature
\texttt{dLQacccjnjs\_aBbB(-5,2)}.
For $M_\mathrm{PMNS} = m003(-2,3)$: 2~tetrahedra, isometry signature
\texttt{cPcbbbdxm\_BaBb(-2,3)}.
Both are the unique triangulations achieving the minimum tetrahedron count,
verified across 200 random retriangulations using SnapPy's
\texttt{randomize()} method (isometry signature invariant in every trial).
The generator presentations $\langle a, b\rangle$ induced by these
triangulations are therefore canonical presentations of
$\pi_1(m006(-5,2))$ and $\pi_1(m003(-2,3))$, and the fitness values
reported here are reproducible by any user of the default SnapPy census.
...
We establish the global minimum of the Borel QR map with respect to
the PDG PMNS matrix. Minimizing $\mathcal{F}$ over all lower-triangular
$L$ with unit diagonal and 3 free off-diagonal entries
$(\ell_{21}, \ell_{31}, \ell_{32} \in \mathbb{R})$
via 50{,}000 Nelder-Mead restarts gives:
\begin{equation}
  \mathcal{F}_\mathrm{min}^\mathrm{Borel} = 0.005087,
\end{equation}
achieved at $(\ell_{21}, \ell_{31}, \ell_{32}) = (-1.131, -1.022, 1.096)$.
This is the global minimum of the Borel QR construction---a property
of the PDG PMNS target matrix, reachable in principle from any manifold.
The geometric significance of $m003(-2,3)$ is therefore not that it
achieves this fitness value, but that it is the unique manifold
satisfying all three selection constraints of
Section~\ref{sec:geometric_selection}.
...
\midrule
m003 (index 1) & $\Zfive$        & 0.981 & \textbf{0.00509} & aa/ab/aB \\
...
m009           & ---             & ---   & 0.02714          & --- \\
\midrule
\multicolumn{5}{l}{\textit{Theoretical minimum (free $L$):} \quad $0.00509$} \\
...
$\{$\texttt{aa}, \texttt{aaB}, \texttt{baa}$\}$ achieves fitness $0.005087$,
the global minimum of the Borel QR construction. The geometric selection
of this manifold is established independently in
Section~\ref{sec:geometric_selection}. The resulting mixing matrix is:
\begin{equation}
|U_\mathrm{geom}| = \begin{pmatrix}
  0.8228 & 0.5480 & 0.1352 \\
  0.3647 & 0.6195 & 0.7052 \\
  0.4358 & 0.5622 & 0.6963
\end{pmatrix}
\label{eq:U_pmns_result}
\end{equation}
compared to the PDG 2024 values~\cite{PDG2024} ($\sin^2\theta_{12}=0.307$, $\sin^2\theta_{23}=0.546$, $\sin^2\theta_{13}=0.02219$, $\delta_\mathrm{CP}=197^\circ$):
\begin{equation}
|U_\mathrm{PMNS}| = \begin{pmatrix}
  0.8232 & 0.5479 & 0.1490 \\
  0.2870 & 0.6195 & 0.7307 \\
  0.4899 & 0.5622 & 0.6663
\end{pmatrix}.
\label{eq:U_pmns_pdg}
\end{equation}
...
\subsubsection*{Parameter-free leptonic CP phase}
The twist angles $\varphi(\gamma) = \mathrm{Im}\log\lambda(\gamma)$
of the three holonomy words give a parameter-free prediction.
The words \texttt{aaB} and \texttt{baa} have $H_1$-classes $1$ and~$3$
(mod~5), summing to $4 \equiv -1$. The real axis triple satisfies
$\det[\hat{n}(\mathrm{aa}), \hat{n}(\mathrm{aaB}), \hat{n}(\mathrm{baa})]
= +0.079 > 0$ (right-handed), fixing the sign contribution to $\pi$.
The twist angles are $\varphi(\mathrm{aaB}) = -176.73^\circ$ and
$\varphi(\mathrm{baa}) = -167.36^\circ$, giving:
\begin{equation}
  \boxed{\delta_{\mathrm{HFG}} = \pi + \varphi(\mathrm{aaB}) +
  \varphi(\mathrm{baa}) \equiv 195.91^\circ \pmod{360^\circ}.}
\end{equation}
This matches the PDG 2024 value $\delta_{\mathrm{PDG}} = 197.0^\circ$
within $1.09^\circ$ ($0.55\%$). No free parameters enter this
derivation; only the holonomy of $m003(-2,3)$ is used.
...
% ============================================================
% ============================================================
\section{Geometric Selection of $M_{\mathrm{PMNS}}$}
\label{sec:geometric_selection}
% ============================================================

The fitness $\mathcal{F} = 0.005087$ is the global minimum of the Borel
QR map and does not by itself identify $m003(-2,3)$ as physically
preferred. The genuine geometric claim is that $m003(-2,3)$ is the
\emph{unique} manifold among all 12 closed orientable hyperbolic
3-manifolds with $H_1 = \mathbb{Z}/5$ in the first 500 census entries
satisfying three independent constraints simultaneously:

\begin{enumerate}
\item \textbf{Lucas-pure covering tower.}
  The only primes appearing in the torsion part of $H_1$ of any finite
  cover of $m003(-2,3)$ through degree~9 are prime Lucas numbers:
  $\mathcal{P} = \{2, 3, 7, 11, 29\}$.
  The prime $13$ does not appear, consistent with $13$ not being a
  Lucas number and the bridge theorem $\ell = 2k\log\varphi
  \Leftrightarrow |\mathrm{tr}(\rho(\gamma))| = L_k$.
  Among the 12 $H_1 = \mathbb{Z}/5$ manifolds, three are Lucas-pure:
  indices~1, 11, and~43.

\item \textbf{The $\sigma_{\mathrm{opt}}$ geodesic.}
  $m003(-2,3)$ contains a geodesic of length
  $\ell_2 = \tfrac{3}{2}\log\varphi = 0.72157$
  (second-shortest geodesic, to $0.03\%$).
  No other $H_1 = \mathbb{Z}/5$ manifold has this geodesic length.
  This length coincides with the Gaussian smearing parameter that
  minimizes the CKM construction fitness on $m006(-5,2)$,
  $\sigma_{\mathrm{opt}}(\mathrm{CKM}) = 0.72157$, providing an
  arithmetic link between the two sectors.

\item \textbf{Minimal volume.}
  $m003(-2,3)$ has volume $0.9814$, the smallest of any closed
  orientable hyperbolic 3-manifold~\cite{GMM}.
\end{enumerate}

Among the 12 $H_1 = \mathbb{Z}/5$ manifolds, only index~1
satisfies both constraints~(1) and~(2). Constraint~(3) is
automatically satisfied by index~1 as the volume-minimum manifold.
This triple coincidence provides a geometric basis for identifying
$m003(-2,3)$ as the lepton-sector manifold, independent of its
fitness to the PDG matrix.
...
\midrule
m003 (idx~1)  & $\mathbb{Z}/5$                          & 0.981 & \textbf{0.00509} & 5 \\
...
PMNS & $N$ (Borel)     & m003 (idx~1)  & 0.981 & $\mathbb{Z}/5$ & 0.00509 \\
\bottomrule
\end{tabular}
\end{center}
...
($H_1 = \mathbb{Z}/5$, fitness 0.00509).
...
The geometric construction achieves $\mathcal{F} = 0.00509$,
...
\subsection{Homological origin of CP suppression in the CKM sector}
\label{subsec:homological_cp}

The suppression of CP violation in the quark sector ($J \approx 0$) is not
a consequence of amphicheirality or any global topological property of $m006$,
but follows from the homology classes of the specific geodesic words
selected by the Frobenius fitness criterion.

For $m006$ (generators $a,b$; relators \texttt{ababbAAbb},
\texttt{abbAbbaBabbAbbAbbaB}), the abelianization map
$\pi_1(M) \twoheadrightarrow H_1(M) \cong \mathbb{Z}/5$
gives $[a] = 2$ and $[b] = 1$.
The flat $\mathrm{U}(1)$ holonomy satisfies
$\chi_k(w) = e^{2\pi i k [w]/5}$.
The optimal CKM triple has homology classes:
\begin{center}\small
\begin{tabular}{lc}
\toprule
...
If two geodesics in the triple satisfy $[\gamma_i] = [\gamma_j]$,
...
For the optimal CKM triple, $[\texttt{AbA}] = [\texttt{AAb}] = 2$,
so two columns are identical and $J = 0$ by the proposition.
This corrects an earlier claim that $J = 0$ follows from
amphicheirality of $m006$; SnapPy confirms $m006$ is in fact
chiral (\texttt{is\_amphicheiral: False}, \texttt{is\_full\_group: True}).
The numerical results are unaffected.
Code: \texttt{analysis/homology\_classes\_manual.py}~\cite{Gentry:Scan}.
...
achieves a better fit (fitness $0.00509$ vs.\ $0.01937$) by
...
$\mathcal{F} = 0.00509$ for PMNS, matching the CKM fitness of
