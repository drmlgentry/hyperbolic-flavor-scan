...
$M_{\mathrm{PMNS}} = \texttt{OrientableClosedCensus[1]}$
($m003(-2,3)$, vol$\approx0.981$, $H_1=\mathbb{Z}/5$), and observe
that every prime appearing in the torsion homology of its finite covers
through degree~9 is a prime Lucas number. The observed prime set is
$\mathcal{P} = \{2,3,5,7,11,29\}$ (excluding the base torsion~5:
$\{2,3,7,11,29\}$), all of which are prime Lucas numbers
$L_k = \varphi^k + \varphi^{-k}$.
The prime $13$ does not appear through degree~9, contrary to an earlier
prediction; since $13$ is not a Lucas number, its absence is consistent
with the Lucas-geodesic bridge theorem $\ell = 2k\log\varphi
\Leftrightarrow |\mathrm{tr}(\rho(\gamma))| = L_k$.

Each prime in $\mathcal{P}$ has an arithmetic source in the Dehn
filling slopes $\mathbf{v}_{\mathrm{PMNS}} = (-2,3)$ and
$\mathbf{v}_{\mathrm{CKM}} = (-5,2)$: slope coordinates give $2$
and $3$; common ancestor torsion gives $5$; coordinate sum gives $7$;
intersection number $\det(\mathbf{v}_{\mathrm{PMNS}},
\mathbf{v}_{\mathrm{CKM}}) = 11$; and $29 = \|\mathbf{v}_{\mathrm{CKM}}\|^2$.

Among all 12 closed hyperbolic 3-manifolds with $H_1=\mathbb{Z}/5$
in the first 500 census entries, only $M_{\mathrm{PMNS}}$ satisfies
both Lucas-purity and the presence of a geodesic of length
$\tfrac{3}{2}\log\varphi$ (the $\sigma_{\mathrm{opt}}$ geodesic),
uniquely selecting it for the lepton sector.
The CKM manifold $M_{\mathrm{CKM}} = \texttt{OrientableClosedCensus[43]}$
is geometrically isolated: it admits no $\mathbb{Z}/5$-preserving
parent in the closed census.
\end{abstract}
...
3-manifolds, with the two flavor manifolds geometrically selected by independent topological and arithmetic invariants:
\begin{align*}
...
\begin{remark}[Geometric uniqueness of the flavor manifolds]
Among all 12 closed orientable hyperbolic 3-manifolds with
$H_1 \cong \mathbb{Z}/5$ in the first 500 census entries,
$M_{\mathrm{PMNS}}$ is the unique manifold satisfying:
(i)~Lucas-pure covering tower through degree~9;
(ii)~geodesic of length $\tfrac{3}{2}\log\phi$ present;
(iii)~smallest volume of any closed hyperbolic 3-manifold.
The CKM manifold $M_{\mathrm{CKM}}$ satisfies (i) and achieves
the best Gaussian kernel fit to PDG CKM among Lucas-pure $\mathbb{Z}/5$
manifolds (fitness 0.016482).
\end{remark}
...
$\geq 7$ & $13$ absent (degree~9 verified) & Not a Lucas number \\
\bottomrule
\end{tabular}
\end{table}
...
\label{conj:main}
Let $\mathcal{P} = \{2,3,5,7,11,29\}$ be as in
...
The conjecture is verified through degree~9 (all covers in the
\\texttt{OrientableClosedCensus} up to index 11{,}031) with zero violations.
The prime $p=13$ does not appear through degree~9. The falsification criterion is explicit: any prime
...
$\mathcal{P} = \{2,3,5,7,11,29\}$ is derived from the slope
...
Third, the absence of $p=13$ through degree~9 is now confirmed.
An analytic proof via the Lucas-geodesic bridge theorem is an open problem.
