...
\author[1]{Marvin L. Gentry\,\thanks{Corresponding author. \texttt{drmlgentry@protonmail.com}. ORCID: 0009-0006-4550-2663.}}
...
\begin{figure}[tp]
\IfFileExists{fig_floor_decay.pdf}{
  \includegraphics[width=0.85\textwidth]{fig_floor_decay.pdf}
}{
  \fbox{\parbox{0.85\textwidth}{\centering\vspace{2cm}
  Figure: Spectral floor decay by homology class.\\
  [fig\_floor\_decay.pdf not included in submission]
  \vspace{2cm}}}
}
...
\subsection{Covering structure and $\mathbb{Z}/5$-preserving towers}
A census scan (\texttt{verify\_covers\_corrected.py}) establishes
that the PMNS manifold \mfld{m003} (\texttt{OrientableClosedCensus[1]})
sits at the base of a $\mathbb{Z}/5$-preserving covering tower:
census index~39 is a degree-2 cover
($H_1=\mathbb{Z}/55\cong\mathbb{Z}/5\times\mathbb{Z}/11$,
vol ratio $= 2.0000000000$ exact, 20 shared geodesics),
and census index~238 is a degree-3 cover
($H_1=\mathbb{Z}/80\cong\mathbb{Z}/16\times\mathbb{Z}/5$,
9 shared geodesics).
By contrast, \mfld{m006} (\texttt{OrientableClosedCensus[43]})
has no $\mathbb{Z}/5$-preserving cover in the closed census:
the unique manifold at double volume (index~1275, $H_1=\mathbb{Z}/3$)
shares only 4 geodesics and does not preserve $\mathbb{Z}/5$.
This covering asymmetry---\mfld{m003} decomposable,
\mfld{m006} irreducible---complements the homology class asymmetry
in the twist spectrum reported in this paper.

\subsection*{SnapPy naming note}
\texttt{OrientableClosedCensus[39]} and
\texttt{OrientableClosedCensus[43]} both carry the name \texttt{m006}
in SnapPy. The manifold used throughout this paper is
\texttt{OrientableClosedCensus[43]} (vol~$=2.0289$, $H_1=\mathbb{Z}/5$).
Manifolds should be referenced by census index, not name string.
...
\bibitem{GentryCKM}
  ...
  Results in Physics, submitted March 2026 (RINP-D-26-00327).
\bibitem{GentryPMNS}
  M.~L.~Gentry,
  ``PMNS Lepton Mixing from the Borel Structure of Hyperbolic Holonomy,''
  Results in Physics, submitted March 2026 (RINP-D-26-00328).
\bibitem{GentryTwist}
  M.~L.~Gentry,
  ``Twist Angle Spectrum of Hyperbolic Holonomy,''
  Results in Physics, submitted March 2026 (RINP-D-26-00330).
