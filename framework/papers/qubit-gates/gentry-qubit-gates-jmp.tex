...
representation $\rho: \pi_1(M) \to \mathrm{PSL}(2,\mathbb{C})$.
Each loxodromic element $\gamma \in \pi_1(M)$ maps under $\rho$
to a matrix with $|\mathrm{Tr}(\rho(\gamma))| > 2$, which via the
identification $\mathrm{PSL}(2,\mathbb{C}) \cong \mathrm{Isom}^+(\mathbb{H}^3)$
acts on $\mathbb{C}^2$ as a rotation of the Bloch sphere
$S^2 \cong \mathrm{SU}(2)/\mathrm{U}(1)$.
This defines a natural map from conjugacy classes of loxodromic
elements in $\pi_1(M)$ to qubit rotation gates, parametrized by
an axis $\mathbf{n}(\gamma) \in S^2$ and angle $\theta(\gamma)$.

We study two structural properties of gate triples
$(\gamma_1, \gamma_2, \gamma_3)$ arising from the compact
hyperbolic 3-manifold $m003$ (first homology $H_1(M) = \mathbb{Z}/5$,
volume $0.981$), and prove two theorems.

\textbf{Theorem 1} (Covering tower invariance).
The gate triple whose Borel lower-triangular construction
reproduces the absolute PMNS lepton mixing matrix
(Frobenius fitness $0.01897$) yields identical unitary matrices
when lifted to the degree-2 and degree-3 covers of $m003$
that preserve $\mathbb{Z}/5$ torsion,
with maximum element deviation below $10^{-4}$.
The gate configuration is therefore a topological invariant of
the $\mathbb{Z}/5$-preserving covering tower.

\textbf{Theorem 2} (Homology class no-go).
For any compact hyperbolic 3-manifold $M$ with
$H_1(M;\mathbb{Z}) = \mathbb{Z}/p$, any gate triple
$(\gamma_1, \gamma_2, \gamma_3)$ in which two words share the
same image under the abelianization map
$\pi_1(M) \to H_1(M;\mathbb{Z}/p)$
satisfies $J = 0$ identically, where $J$ is the
Jarlskog CP-violation invariant of the resulting unitary matrix.
This provides a topological obstruction to CP-violating
quantum operations arising from homology class collision.

The CKM quark sector realizes Theorem~2 exactly: its optimal gate
triple for the manifold $m006(-5,2)$ has two words with identical
homology class, forcing $J = 0$ at leading order.

The proofs use the Borel lower-triangular construction,
the abelianization of $\pi_1(M)$, and SnapPy~\cite{SnapPy}
census computations.
A connection to topological quantum computing~\cite{Kitaev2003,
Nayak2008} is discussed in Section~\ref{sec:discussion},
where we note that Theorem~1 provides a form of discrete
topological protection distinct from braid-based schemes.
