...
def scan_cusp(cusp_name, max_abs=5, cutoff=4.5, tol=0.02):
    ...
    sys.stdout.write(f"Total slopes: {total}\n")
    ...
    for idx, (p,q) in enumerate(slopes):
        try:
            ...
            vol = float(M.volume())
            if vol < 0.5:
                sys.stdout.write(f"[{idx+1:3d}/{total}] ({p:2d},{q:2d}) -> non-hyperbolic (vol={vol:.3f})\n")
                sys.stdout.flush()
                continue
            ...
            sys.stdout.write(f"[{idx+1:3d}/{total}] ({p:2d},{q:2d}) vol={vol:.4f} hits={len(hits):2d}  | hyp={len(hyperbolic)}  eta={eta:.0f}s\n")
            sys.stdout.flush()
...
# Run scans
sys.stdout.write("\n" + "="*60 + "\n")
sys.stdout.write("FLUSH SCAN: cutoff=4.5, tolerance=0.02\n")
sys.stdout.write("="*60 + "\n")
sys.stdout.flush()
