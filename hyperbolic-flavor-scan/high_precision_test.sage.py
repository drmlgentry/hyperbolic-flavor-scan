...
# This file was *autogenerated* from the file high_precision_test.sage
from sage.all_cmdline import *   # import sage library

_sage_const_65 = Integer(65); _sage_const_1000 = Integer(1000); _sage_const_1 = Integer(1); _sage_const_2 = Integer(2); _sage_const_43 = Integer(43); _sage_const_3 = Integer(3); _sage_const_17 = Integer(17); _sage_const_4p18 = RealNumber('4.18'); _sage_const_1p27 = RealNumber('1.27'); _sage_const_100 = Integer(100); _sage_const_500 = Integer(500); _sage_const_0p8 = RealNumber('0.8'); _sage_const_0 = Integer(0); _sage_const_5 = Integer(5); _sage_const_1p7769 = RealNumber('1.7769'); _sage_const_0p10566 = RealNumber('0.10566')
import snappy
...
def get_mat_hp(rho, word, prec=_sage_const_1000 ):
    """Get normalized holonomy matrix at high precision."""
    CC_hp = ComplexField(prec)
    mat = matrix(CC_hp, [[CC_hp(rho(word)[i,j]) for j in range(_sage_const_2 )]
                          for i in range(_sage_const_2 )])
    d = mat.det()
    return mat / d.sqrt()

def dominant_eig_hp(rho, word, prec=_sage_const_1000 ):
    """
...
print(f"  ell_a = 2*log|lam_dom| = {_sage_const_2 *log(abs(lam_a_dom))}")
print()
print(f"Generator b:")
print(f"  |lambda_dom| = {abs(lam_b_dom)}")
print(f"  |lambda_sub| = {abs(lam_b_sub)}")
print(f"  |lam_dom|*|lam_sub| = {prod_b}  (should be 1.000...)")
print(f"  ell_b = 2*log|lam_dom| = {_sage_const_2 *log(abs(lam_b_dom))}")
print()

ell_a = _sage_const_2 *log(abs(lam_a_dom))
ell_b = _sage_const_2 *log(abs(lam_b_dom))
...
three_minus_sqrt17 = RR(_sage_const_3 ) - RR(_sage_const_17 ).sqrt()
...
n_correct_bits_calib = -log(diff_calib/abs(three_minus_sqrt17))/log(RR(_sage_const_2 ))
...
ratio = lam_b_dom**_sage_const_2  / lam_a_dom
ratio_sq = abs(ratio)**_sage_const_2 

mb_mc_pdg = RR(_sage_const_4p18 ) / RR(_sage_const_1p27 )
...
print(f"Error in percent        = {float(rel_err)*_sage_const_100 }%")
print()

n_correct_bits = -log(rel_err)/log(RR(_sage_const_2 ))
print(f"Agreement to ~{float(n_correct_bits):.1f} bits")
print()

print("INTERPRETATION:")
if float(n_correct_bits) > _sage_const_500 :
    print("  EXACT RELATION: agrees to 500+ bits")
    print("  |lambda_b^2/lambda_a|^2 = mb/mc is a THEOREM")
elif float(n_correct_bits) > float(n_correct_bits_calib) * _sage_const_0p8 :
...
if first_diff:
    ...
    print(f"  ratio^2: ...{r_str[max(_sage_const_0 ,first_diff-_sage_const_3 ):first_diff+_sage_const_5 ]}...")
    print(f"  mb/mc:   ...{m_str[max(_sage_const_0 ,first_diff-_sage_const_3 ):first_diff+_sage_const_5 ]}...")
print()
...
ell_a_C = _sage_const_2 *log(abs(lam_a_C_dom))
exp_3a = exp(_sage_const_3 *ell_a_C)
mtau_mmu = RR(_sage_const_1p7769 )/RR(_sage_const_0p10566 )

print(f"exp(3*ell_a on m006) = {exp_3a}")
print(f"mtau/mmu (PDG)       = {mtau_mmu}")
diff2 = abs(exp_3a - mtau_mmu)
rel_err2 = diff2/mtau_mmu
print(f"Relative error       = {float(rel_err2)*_sage_const_100 }%")
n_bits2 = -log(rel_err2)/log(RR(_sage_const_2 ))
...
ell_b_P = ell_b
exp_2a_b = exp(_sage_const_2 *ell_a + ell_b_P)
print(f"exp(2*ell_a + ell_b on m003) = {exp_2a_b}")
print(f"mtau/mmu (PDG)               = {mtau_mmu}")
diff3 = abs(exp_2a_b - mtau_mmu)
rel_err3 = diff3/mtau_mmu
print(f"Relative error               = {float(rel_err3)*_sage_const_100 }%")
n_bits3 = -log(rel_err3)/log(RR(_sage_const_2 ))
