...
# ---- Column names (from your scan pipeline) ----
COL_LEN = "length"
COL_CLASS = "h1_class"
...
def running_min_curve(d):
    ...
    return g[COL_LEN].to_numpy(), g["phi_min"].to_numpy()
...
for k in sorted(df[COL_CLASS].unique()):
    ...
    plt.title(f"m006 — Class {k} decay")
    ...
    summary.append((k, cexp, apow, aic_exp, aic_pow))
...
for k, cexp, apow, aic_exp, aic_pow in summary:
    ...
    print(f"{k:5d} | {cexp:6.4f} | {apow:9.4f} | {aic_exp:8.2f} | {aic_pow:8.2f} | {pref}")
