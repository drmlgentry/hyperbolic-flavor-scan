...
def axis_from_SL2C(A):
    ...
    if abs(c) < 1e-10:
        ...
        disc = (d-a)**2 + 4*b*c
        sq = np.sqrt(disc+0j)
        z1=(-(d-a)+sq)/(2*c); z2=(-(d-a)-sq)/(2*c)
        z = z1 if abs(z1)>=abs(z2) else z2
    if abs(z)>1e8: return np.array([0.,0.,1.])
    x,y=z.real,z.imag; r2=x*x+y*y; d2=1+r2
...
PMNS_exp=np.array([[0.821,0.551,0.150],[0.358,0.694,0.624],[0.444,0.461,0.767]])
CKM_exp=np.array([[0.974,0.225,0.004],[0.225,0.973,0.041],[0.009,0.040,0.999]])
...
for label,mname,filling,exp_matrix,sigma_opt in [
    ...
    print(f"\n{'='*60}", flush=True)
    print(f"  {label}: {mname}{filling}  sigma_opt={sigma_opt:.5f}", flush=True)
    print(f"{'='*60}", flush=True)

    M=snappy.Manifold(mname); M.dehn_fill(filling)
    G=M.fundamental_group(simplify_presentation=True)
    gens=G.generators(); PH=M.polished_holonomy()
        ...
        for wt in itertools.product(letters,repeat=length):
            ...
            try:
                ...
                if not any(abs(np.dot(n,m))>0.9999 for m in all_axes):
                    all_axes.append(n); all_words.append(word)
            except: pass
    print(f"  Unique axes: {len(all_axes)}", flush=True)

    best_fit=1e9; best_triple=None; best_axes=None
    for idx in itertools.combinations(range(len(all_axes)),3):
        ...
        if f<best_fit:
            best_fit=f; best_triple=tuple(all_words[i] for i in idx)
            best_axes=axes3.copy()

    print(f"  Best triple: {best_triple}  fitness={best_fit:.6f}", flush=True)
    for i,(w,n) in enumerate(zip(best_triple,best_axes)):
        print(f"    [{i}] '{w}': {np.round(n,4)}", flush=True)

    print(f"  Pairwise angles:", flush=True)
    for i in range(3):
        for j in range(i+1,3):
            th=angle_between(best_axes[i],best_axes[j])
            print(f"    {i} vs {j}: {np.degrees(th):.2f} deg", flush=True)

    # Full kernel rank sweep
    print(f"\n  {'sigma':>8} {'R_eff':>8} {'E3':>8} {'E2':>8}  note", flush=True)
    print("  "+"-"*50, flush=True)
    for sigma in np.arange(0.20,1.31,0.10):
        K=build_kernel(best_axes,sigma)
        R=eff_rank(K); E3=energy_frac(K,3); E2=energy_frac(K,2)
        note=''
    ...
    # Eigenvalues at sigma_opt (only 3 eigenvalues for 3x3)
    K_opt=build_kernel(best_axes,sigma_opt)
    ev=np.sort(np.linalg.eigvalsh(K_opt))[::-1]
    ev_n=ev/ev[0]
    print(f"\n  Eigenvalues at sigma_opt:", flush=True)
    for i in range(3):
        gap=ev_n[i]/ev_n[i+1] if i<2 else 0.0
        print(f"    lambda_{i}={ev_n[i]:.6f}  gap_to_next={gap:.4f}",
              flush=True)
    print(f"  R_eff={eff_rank(K_opt):.5f}", flush=True)
    print(f"  E(top2)={energy_frac(K_opt,2):.5f}", flush=True)

    # Reconstructed mixing matrix
    Q2,R2=np.linalg.qr(K_opt)
    for i in range(3):
        if R2[i,i]<0: Q2[:,i]=-Q2[:,i]
    U=np.abs(Q2)
    print(f"\n  |U| reconstructed:", flush=True)
    for row in U: print(f"    {np.round(row,4)}", flush=True)
    print(f"  |U| experimental:", flush=True)
    for row in exp_matrix: print(f"    {np.round(row,4)}", flush=True)
    print(f"  Final RMS: {np.sqrt(np.mean((U-exp_matrix)**2)):.6f}",
          flush=True)
